...
class RadarNet:
    ...
    def __init__(self, seq_length):
        ...
        * Output layer
        ...
        # Input manipulation
        x = tf.placeholder(tf.float32, shape=[seq_length, 232, 232])
        ...
        # RNN Layer
        h_unstack3 = tf.unstack(tf.reshape(
            h_relu2, [seq_length, 1, 256]), axis=0)
        ...
        W_out4 = tf.Variable(tf.truncated_normal(
            [1, 50], stddev=1 / np.sqrt(50)))
        b_out4 = tf.Variable(tf.truncated_normal([1], stddev=0.1))
        y_unstack = [W_out4 @ tf.reshape(h_outputs3[i], [50, 1]) + b_out4
                     for i in range(seq_length)]
        y = tf.tanh(tf.reshape(tf.stack(y_unstack), [seq_length]))

        # Loss, optimization
        y_ = tf.placeholder(tf.float32, shape=[seq_length])
    ...
    def tensors(self):
        return self.x, self.y, self.y_, self.loss, self.optimize
